...
export default function App() {
  ...
  const fetchVortexData = async () => {
    if (!API_KEY) {
      setLogs(p => [...p, { t: "KRİTİK HATA: API Anahtarı Bulunamadı!", c: "#ff0000" }]);
      return;
    }

    setLoading(true);
    setLogs(p => [...p, { t: "Gemini AI ile bağlantı kuruluyor...", c: "#888" }]);

    try {
      const { text } = await generateText({
        model: google('models/gemini-1.5-flash'), // SDK 4 için en güvenli yazım
        prompt: 'Teknoloji dünyasından 5 kısa gelişme ver. JSON: [{"t": "Başlık", "a": "Analiz"}]',
      });

      const jsonMatch = text.match(/\[.*\]/s);
      if (jsonMatch) {
        setNodes(JSON.parse(jsonMatch[0]));
        setLogs(p => [...p, { t: "VORTEX: AI Sinyali Stabil.", c: "#00ff00" }]);
      }
    } catch (e) {
    ...
    <div style={{ background: '#020205', color: '#fff', height: '100vh', display: 'flex', flexDirection: 'column', fontFamily: 'monospace' }}>
      <header style={{ height: '70px', borderBottom: '1px solid #111', display: 'flex', alignItems: 'center', padding: '0 30px', justifyContent: 'space-between', background: '#000' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <h1 style={{ letterSpacing: '5px', fontWeight: '900', fontSize: '18px' }}>VORTEX CORE</h1>
        </div>
        <div style={{ fontSize: '10px', color: '#444' }}>STATUS: ONLINE // AI: CONNECTED</div>
      </header>
        ...
        <main style={{ flex: 1, padding: '30px', overflowY: 'auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' }}>
          {nodes.map((n, i) => (
            <div key={i} style={{ padding: '20px', background: '#05050a', border: '1px solid #111', borderRadius: '10px', borderLeft: '3px solid #0055ff' }}>
              ...
              <p style={{ color: '#666', fontSize: '13px', lineHeight: '1.5' }}>{n.a}</p>
            </div>
          ))}
