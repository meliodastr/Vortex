...
export default function App() {
  ...
  const [logs, setLogs] = useState([{ t: "VORTEX_v15: Sistem Çekirdeği Aktif.", c: "#0055ff" }]);
  ...
  const fetchVortex = async () => {
    if (!API_KEY) {
      setLogs(p => [...p, { t: "KRİTİK: API_KEY EKSİK!", c: "#ff0000" }]);
      return;
    }
    setLoading(true);
    setLogs(p => [...p, { t: "AI Hattı Taranıyor...", c: "#888" }]);

    try {
      const { text } = await generateText({
        model: google('gemini-1.5-flash-latest'),
        prompt: 'Bana 5 teknoloji gelişmesi ver. Format: [{"t": "Başlık", "a": "Analiz"}]'
      });
      ...
      setLogs(p => [...p, { t: `HATA: ${e.message.slice(0, 40)}`, c: "#ffaa00" }]);
    ...
    <div style={{ background: '#020205', color: '#fff', height: '100vh', display: 'flex', flexDirection: 'column', fontFamily: 'monospace' }}>
      <header style={{ height: '70px', borderBottom: '1px solid #111', display: 'flex', alignItems: 'center', padding: '0 30px', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <h1 style={{ letterSpacing: '5px' }}>VORTEX CORE</h1>
        </div>
        <div style={{ fontSize: '10px', color: '#444' }}>STATUS: {loading ? 'SCANNING' : 'ONLINE'}</div>
      </header>
        ...
        <main style={{ flex: 1, padding: '30px', overflowY: 'auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
          {nodes.map((n, i) => (
            <div key={i} style={{ padding: '20px', background: '#05050a', border: '1px solid #111', borderRadius: '10px' }}>
              ...
              <h3 style={{ marginBottom: '10px', color: '#eee' }}>{n.t}</h3>
